...
*/
	if(isset($_SESSION['check']) && $_SESSION['check'] == 1)
	{
		...
		echo '<input id="dropdown" type="hidden" name="dropdown" value="'.$_REQUEST['dropdown'].'"/>';
		else
		echo '<input id="dropdown" type="hidden" name="dropdown" value="'.$_REQUEST['dropdown'].'"/>';
		*/
...
//		echo '<table>';
		...
		{
			if(isset($_POST["i".$i]))
			echo '<td>'.$row2['stdnt_id']." ".'</td><td>'.$row2['stdnt_name'].'</td>'.'<td><input  id="textbox" type="text" value= "'.$_POST["i".$i]. '" name= "i'.$i.'"/></td>';
			else
